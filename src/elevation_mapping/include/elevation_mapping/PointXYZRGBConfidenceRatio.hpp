...
struct
    _PointXYZRGBConfidenceRatio {  // NOLINT(cppcoreguidelines-pro-type-union-access)
  PCL_ADD_POINT4D;  // NOLINT(cppcoreguidelines-pro-type-union-access,
  ...
  union {
    ...
    float data_c[4];  // NOLINT(readability-identifier-naming,
...
struct PointXYZRGBConfidenceRatio : public _PointXYZRGBConfidenceRatio {
  inline explicit PointXYZRGBConfidenceRatio(
      const _PointXYZRGBConfidenceRatio& p)
    ...
    // Confidence
    confidence_ratio =
        p.confidence_ratio;  // NOLINT(cppcoreguidelines-pro-type-union-access)
  }
  ...
  inline PointXYZRGBConfidenceRatio(std::uint8_t _r, std::uint8_t _g,
                                    std::uint8_t _b)
      : PointXYZRGBConfidenceRatio(0.f, 0.f, 0.f, _r, _g, _b) {}

  inline PointXYZRGBConfidenceRatio(float _x, float _y, float _z)
      : PointXYZRGBConfidenceRatio(_x, _y, _z, 0, 0, 0) {}

  inline PointXYZRGBConfidenceRatio(float _x, float _y, float _z,
                                    std::uint8_t _r, std::uint8_t _g,
                                    std::uint8_t _b,
    ...
    confidence_ratio =
        _confidence_ratio;  // NOLINT(cppcoreguidelines-pro-type-union-access)
  }

  friend std::ostream& operator<<(std::ostream& os,
                                  const PointXYZRGBConfidenceRatio& p);
};
...
POINT_CLOUD_REGISTER_POINT_STRUCT(
    pcl::_PointXYZRGBConfidenceRatio,  // NOLINT(modernize-avoid-c-arrays,
...
POINT_CLOUD_REGISTER_POINT_WRAPPER(pcl::PointXYZRGBConfidenceRatio,
                                   pcl::_PointXYZRGBConfidenceRatio)
