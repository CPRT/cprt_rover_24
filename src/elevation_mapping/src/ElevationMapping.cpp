...
ElevationMapping::ElevationMapping(std::shared_ptr<rclcpp::Node> nodeHandle)
  ...
  RCLCPP_WARN(nodeHandle_->get_logger(),
              "CMake Build Type is 'Debug'. Change to 'Release' for better "
              "performance.");
#endif
  ...
  transformListener_ =
      std::make_shared<tf2_ros::TransformListener>(*transformBuffer_);
...
void ElevationMapping::setupServices() {
  ...
  fusionTriggerService_ = nodeHandle_->create_service<std_srvs::srv::Empty>(
      ...
      std::bind(&ElevationMapping::fuseEntireMapServiceCallback, this,
                ...
                std::placeholders::_3),
      rclcpp::ServicesQoS().get_rmw_qos_profile(), fusionServiceGroup_);

  fusedSubmapService_ =
      nodeHandle_->create_service<grid_map_msgs::srv::GetGridMap>(
          "get_submap",
  ...
  clearMapService_ = nodeHandle_->create_service<std_srvs::srv::Empty>(
      "clear_map", std::bind(&ElevationMapping::clearMapServiceCallback, this,
                             std::placeholders::_1, std::placeholders::_2,
                             std::placeholders::_3));
  ...
  maskedReplaceService_ =
      nodeHandle_->create_service<grid_map_msgs::srv::SetGridMap>(
          "masked_replace",
          std::bind(&ElevationMapping::maskedReplaceServiceCallback, this,
                    std::placeholders::_1, std::placeholders::_2));
  saveMapService_ =
      nodeHandle_->create_service<grid_map_msgs::srv::ProcessFile>(
          "save_map", std::bind(&ElevationMapping::saveMapServiceCallback, this,
                                std::placeholders::_1, std::placeholders::_2));
  loadMapService_ =
      nodeHandle_->create_service<grid_map_msgs::srv::ProcessFile>(
          "load_map", std::bind(&ElevationMapping::loadMapServiceCallback, this,
                                std::placeholders::_1, std::placeholders::_2));
}
...
bool ElevationMapping::readParameters() {
  ...
  if (minUpdateRate == 0.0) {
    ...
    RCLCPP_WARN(nodeHandle_->get_logger(),
                "Rate for publishing the map is zero.");
  } else {
  ...
  nodeHandle_->get_parameter("fused_map_publishing_rate",
                             fusedMapPublishingRate);
  if (fusedMapPublishingRate == 0.0) {
    fusedMapPublishTimerDuration_ = rclcpp::Duration::from_seconds(0.0);
    RCLCPP_WARN(
        nodeHandle_->get_logger(),
        "Rate for publishing the fused map is zero. The fused elevation map "
        "will not be published unless the service `triggerFusion` is "
        "called.");
    ...
    fusedMapPublishTimerDuration_ =
        rclcpp::Duration::from_seconds(1.0 / fusedMapPublishingRate);
  }
  ...
  if (visibilityCleanupRate == 0.0) {
    visibilityCleanupTimerDuration_ = rclcpp::Duration::from_seconds(0.0);
    RCLCPP_WARN(nodeHandle_->get_logger(),
                "Rate for visibility cleanup is zero and therefore disabled.");
  } else {
    visibilityCleanupTimerDuration_ =
        rclcpp::Duration::from_seconds(1.0 / visibilityCleanupRate);
  ...
  nodeHandle_->get_parameter("mahalanobis_distance_threshold",
  ...
  nodeHandle_->get_parameter("enable_visibility_cleanup",
                             map_.enableVisibilityCleanup_);
  ...
  nodeHandle_->declare_parameter("target_frame_init_submap",
                                 std::string("/footprint"));

  nodeHandle_->get_parameter("initialize_elevation_map",
                             initializeElevationMap_);
  ...
  nodeHandle_->get_parameter("init_submap_height_offset",
                             initSubmapHeightOffset_);
  nodeHandle_->get_parameter("target_frame_init_submap",
                             targetFrameInitSubmap_);
...
void ElevationMapping::pointCloudCallback(
  ...
  if (!receivedFirstMatchingPointcloudAndPose_) {
    ...
    const double currentPointCloudTime =
        rclcpp::Time(pointCloudMsg->header.stamp, RCL_ROS_TIME).seconds();

    if (currentPointCloudTime < oldestPoseTime) {
      auto clock = nodeHandle_->get_clock();
      RCLCPP_WARN_THROTTLE(
          nodeHandle_->get_logger(), *(clock), 5,
          "No corresponding point cloud and pose are found. Waiting for first "
          "match. (Warning message is throttled, 5s.)");
      return;
    } else {
      RCLCPP_INFO(nodeHandle_->get_logger(),
                  "First corresponding point cloud and pose found, elevation "
                  "mapping started. ");
  ...
  if (!ignoreRobotMotionUpdates_) {
    std::shared_ptr<const nav_msgs::msg::Odometry> poseMessage =
        robotPoseCache_.getElemBeforeTime(lastPointCloudUpdateTime_);
    if (!poseMessage) {
      // Tell the user that either for the timestamp no pose is available or
      // that the buffer is possibly empty
      if (robotPoseCache_.getOldestTime().seconds() >
          lastPointCloudUpdateTime_.seconds()) {
        RCLCPP_ERROR(nodeHandle_->get_logger(),
                     "The oldest pose available is at %f, requested pose at %f",
                     robotPoseCache_.getOldestTime().seconds(),
                     lastPointCloudUpdateTime_.seconds());
      } else {
        RCLCPP_ERROR(nodeHandle_->get_logger(),
                     "Could not get pose information from robot for time %f. "
                     "Buffer empty?",
                     lastPointCloudUpdateTime_.seconds());
      }
      return;
  ...
  if (!sensorProcessor_->process(pointCloud, robotPoseCovariance,
                                 pointCloudProcessed, measurementVariances,
    ...
    if (!sensorProcessor_->isTfAvailableInBuffer()) {
      ...
      RCLCPP_INFO_THROTTLE(nodeHandle_->get_logger(), clock, 10,
                           "Waiting for tf transformation to be available. "
                           "(Message is throttled, 10s.)");
      return;
    }
    RCLCPP_ERROR(
  ...
  if (map_.enableContinuousCleanup_) {
    RCLCPP_DEBUG(nodeHandle_->get_logger(),
                 "Clearing elevation map before adding new point cloud.");
    map_.clear();
  }

  // Add point cloud to elevation map.
  if (!map_.add(
          pointCloudProcessed, measurementVariances, lastPointCloudUpdateTime_,
...
void ElevationMapping::publishFusedMapCallback() {
  ...
  RCLCPP_DEBUG(nodeHandle_->get_logger(),
               "Elevation map is fused and published from timer.");
...
bool ElevationMapping::updatePrediction(const rclcpp::Time& time) {
  ...
  RCLCPP_DEBUG(nodeHandle_->get_logger(),
               "Updating map with latest prediction from time %f.",
               robotPoseCache_.getLatestTime().seconds());

  if (time + timeTolerance_ < map_.getTimeOfLastUpdate()) {
    RCLCPP_ERROR(
        nodeHandle_->get_logger(),
        "Requested update with time stamp %f, but time of last update was %f.",
        time.seconds(), map_.getTimeOfLastUpdate().seconds());
    return false;
  } else if (time < map_.getTimeOfLastUpdate()) {
    RCLCPP_DEBUG(nodeHandle_->get_logger(),
                 "Requested update with time stamp %f, but time of last update "
                 "was %f. Ignoring update.",
                 time.seconds(), map_.getTimeOfLastUpdate().seconds());
    return true;
  }
  ...
  std::shared_ptr<const nav_msgs::msg::Odometry> poseMessage =
      robotPoseCache_.getElemBeforeTime(time);
  if (!poseMessage) {
    // Tell the user that either for the timestamp no pose is available or that
    // the buffer is possibly empty
    if (robotPoseCache_.getOldestTime().seconds() >
        lastPointCloudUpdateTime_.seconds()) {
      RCLCPP_ERROR(nodeHandle_->get_logger(),
                   "The oldest pose available is at %f, requested pose at %f",
  ...
  // Covariance is stored in row-major in ROS:
...
bool ElevationMapping::updateMapLocation() {
  RCLCPP_DEBUG(nodeHandle_->get_logger(),
               "Elevation map is checked for relocalization.");
...
bool ElevationMapping::getFusedSubmapServiceCallback(
  ...
  bool isSuccess;
  grid_map::GridMap subMap = map_.getFusedGridMap().getSubmap(
  ...
  RCLCPP_DEBUG(nodeHandle_->get_logger(),
               "Elevation submap responded with timestamp %f.",
               map_.getTimeOfLastFusion().seconds());
  return isSuccess;
}

bool ElevationMapping::getRawSubmapServiceCallback(
    std::shared_ptr<grid_map_msgs::srv::GetGridMap::Request> request,
    std::shared_ptr<grid_map_msgs::srv::GetGridMap::Response> response) {
  grid_map::Position requestedSubmapPosition(request->position_x,
                                             request->position_y);
  ...
  RCLCPP_DEBUG(
      nodeHandle_->get_logger(),
      "Elevation raw submap request: Position x=%f, y=%f, Length x=%f, y=%f.",
      requestedSubmapPosition.x(), requestedSubmapPosition.y(),
      requestedSubmapLength(0), requestedSubmapLength(1));
  boost::recursive_mutex::scoped_lock scopedLock(map_.getRawDataMutex());

  bool isSuccess;
  grid_map::GridMap subMap = map_.getRawGridMap().getSubmap(
      requestedSubmapPosition, requestedSubmapLength, isSuccess);
  scopedLock.unlock();
...
bool ElevationMapping::disableUpdatesServiceCallback(
    const std::shared_ptr<rmw_request_id_t>,
    const std::shared_ptr<std_srvs::srv::Empty::Request>,
    std::shared_ptr<std_srvs::srv::Empty::Response>) {
...
bool ElevationMapping::enableUpdatesServiceCallback(
    const std::shared_ptr<rmw_request_id_t>,
    const std::shared_ptr<std_srvs::srv::Empty::Request>,
    std::shared_ptr<std_srvs::srv::Empty::Response>) {
...
bool ElevationMapping::initializeElevationMap() {
  if (initializeElevationMap_) {
    if (static_cast<elevation_mapping::InitializationMethods>(
            initializationMethod_) ==
        elevation_mapping::InitializationMethods::PlanarFloorInitializer) {
      geometry_msgs::msg::TransformStamped transform_msg;
      tf2::Stamped<tf2::Transform> transform;

      // Listen to transform between mapFrameId_ and targetFrameInitSubmap_ and
      // use z value for initialization
      try {
        transform_msg = transformBuffer_->lookupTransform(
            mapFrameId_, targetFrameInitSubmap_, rclcpp::Time(0),
        ...
        // Move map before we apply the height values. This prevents unwanted
        // behavior from intermediate move() calls in updateMapLocation().
        map_.move(positionRobot);

        map_.setRawSubmapHeight(
        ...
        RCLCPP_WARN(
            nodeHandle_->get_logger(),
            "Could not initialize elevation map with constant height. (This "
            "warning can be ignored if TF tree is not available.)");
        return false;
      }
    }
  }
...
bool ElevationMapping::clearMapServiceCallback(
    const std::shared_ptr<rmw_request_id_t>,
    const std::shared_ptr<std_srvs::srv::Empty::Request>,
    std::shared_ptr<std_srvs::srv::Empty::Response>) {
...
bool ElevationMapping::maskedReplaceServiceCallback(
  ...
  if (sourceMap.exists(maskedReplaceServiceMaskLayerName_)) {
    ...
    mask =
        Eigen::MatrixXf::Ones(sourceMap.getSize()(0), sourceMap.getSize()(1));
  }

  boost::recursive_mutex::scoped_lock scopedLockRawData(map_.getRawDataMutex());

  // Loop over all layers that should be set
  for (auto sourceLayerIterator = sourceMap.getLayers().begin();
       sourceLayerIterator != sourceMap.getLayers().end();
       sourceLayerIterator++) {
    // skip "mask" layer
    ...
    if (map_.getRawGridMap().exists(*sourceLayerIterator)) {
      ...
      for (grid_map::GridMapIterator destinationIterator(map_.getRawGridMap());
        ...
        // Use the position to find corresponding indices in source and
        // destination
        ...
        if (!std::isnan(mask(sourceIndex(0), sourceIndex(1)))) {
          destinationLayer(destinationIndex(0), destinationIndex(1)) =
              sourceLayer(sourceIndex(0), sourceIndex(1));
        }
      }
    } else {
      RCLCPP_ERROR(nodeHandle_->get_logger(),
                   "Masked replace service: Layer %s does not exist!",
                   sourceLayerIterator->c_str());
    }
  }
...
bool ElevationMapping::saveMapServiceCallback(
  ...
  map_.fuseAll();
  std::string topic =
      std::string(nodeHandle_->get_namespace()) + "/elevation_map";
  if (!request->topic_name.empty()) {
    topic =
        std::string(nodeHandle_->get_namespace()) + "/" + request->topic_name;
  }
  response->success =
      static_cast<unsigned char>(grid_map::GridMapRosConverter::saveToBag(
          map_.getFusedGridMap(), request->file_path, topic));
  response->success = static_cast<unsigned char>(
      (grid_map::GridMapRosConverter::saveToBag(
          map_.getRawGridMap(), request->file_path + "_raw", topic + "_raw")) &&
      static_cast<bool>(response->success));
  return static_cast<bool>(response->success);
}

bool ElevationMapping::loadMapServiceCallback(
    std::shared_ptr<grid_map_msgs::srv::ProcessFile::Request> request,
    std::shared_ptr<grid_map_msgs::srv::ProcessFile::Response> response) {
  ...
  response->success =
      static_cast<unsigned char>(grid_map::GridMapRosConverter::loadFromBag(
          request->file_path, topic, map_.getFusedGridMap()));
  response->success = static_cast<unsigned char>(
      grid_map::GridMapRosConverter::loadFromBag(
          request->file_path + "_raw", topic + "_raw", map_.getRawGridMap()) &&
      static_cast<bool>(response->success));
