...
bool ElevationMap::add(const PointCloudType::Ptr pointCloud,
  ...
  if (use_gpu_) {
    elevationMappingGPU_.updateMapGPU(
        ...
        minHorizontalVariance_, multiHeightNoise_,
        mahalanobisDistanceThreshold_, scanningDuration_, rawMap_);
  } else {
    ...
    for (unsigned int i = 0; i < pointCloud->size(); ++i) {
      ...
      if (!rawMap_.getIndex(position, index)) {
        continue;  // Skip this point if it does not lie within the elevation
                   // map.
      }
      ...
      auto& horizontalVarianceXY =
          horizontalVarianceXYLayer(index(0), index(1));
      ...
      // Variance: %f", elevation, point.z, variance); Deal with multiple
      // heights in one cell.
      ...
      const float pointHeightPlusUncertainty =
          point.z + 3.0 * sqrt(pointVariance);  // 3 sigma
      if (std::isnan(lowestScanPoint) ||
          pointHeightPlusUncertainty < lowestScanPoint) {
        // RCLCPP_INFO(nodeHandle_->get_logger(), "Updating lowest scan
        // point.");
      ...
      // RCLCPP_INFO(nodeHandle_->get_logger(), "Fusing measurement with
      // elevation map data."); Fuse measurement with elevation map data. aka
      // kalman filter
      elevation =
      ...
      horizontalVarianceY = minHorizontalVariance_;
      horizontalVarianceXY = 0.0;
      // RCLCPP_INFO(nodeHandle_->get_logger(), "Updated map cell: Elevation =
      // %f, Variance = %f", elevation, variance);
    }
  }
...
bool ElevationMap::update(const Pose& robotPose,
  ...
  grid_map::Size size = rawMap_.getSize();
  ...
  computeReducedCovariance(robotPose, robotPoseCovarianceScaled,
                           reducedCovariance);
  ...
  kindr::RotationMatrixPD mapToRobotRotation = kindr::RotationMatrixPD(
      robotPose.getRotation().inverted() * getPose().getRotation());
  kindr::RotationMatrixPD mapToPreviousRobotRotationInverted =
      kindr::RotationMatrixPD(previousRobotPose_.getRotation().inverted() *
                              getPose().getRotation())
          .inverted();

  // Translation Jacobian (J_r) (25).
  Eigen::Matrix3d translationJacobian =
      -mapToRobotRotation.matrix().transpose();

  // Translation variance update (for all points the same).
  Eigen::Vector3f translationVarianceUpdate =
      (translationJacobian * positionCovariance *
       translationJacobian.transpose())
          .diagonal()
          .cast<float>();
  ...
  const kindr::Position3D positionRobotToMap =
      getPose().getRotation().inverseRotate(getPose().getPosition() -
                                            previousRobotPose_.getPosition());

  auto& heightLayer = rawMap_["elevation"];
  ...
  for (unsigned int i = 0; i < static_cast<unsigned int>(size(0)); ++i) {
    for (unsigned int j = 0; j < static_cast<unsigned int>(size(1)); ++j) {
      ...
      if (std::isfinite(height)) {
        grid_map::Position position;
        rawMap_.getPosition({i, j}, position);
        ...
        const Eigen::Matrix2f rotationVarianceUpdate =
            (rotationJacobian * rotationCovariance *
        ...
        horizontalVarianceUpdateX(i, j) =
            translationVarianceUpdate.x() + rotationVarianceUpdate(0, 0);
        horizontalVarianceUpdateY(i, j) =
            translationVarianceUpdate.y() + rotationVarianceUpdate(1, 1);
        ...
        horizontalVarianceUpdateY(i, j) =
            std::numeric_limits<float>::infinity();
        horizontalVarianceUpdateXY(i, j) =
            std::numeric_limits<float>::infinity();
      }
    }
  }

  update_variance_layers(varianceUpdate, horizontalVarianceUpdateX,
                         horizontalVarianceUpdateY, horizontalVarianceUpdateXY,
                         time);
...
bool ElevationMap::update_variance_layers(
...
bool ElevationMap::computeReducedCovariance(
  ...
  // (A.5)
  Eigen::Matrix<double, 1, 3> yawJacobian(cos(eulerAngles.yaw()) * tanOfPitch,
...
bool ElevationMap::computeRelativeCovariance(
  ...
  const kindr::RotationVectorPD rotationVector_I_tilde_B(
      0.0, 0.0, rotationVector_I_B.vector().z());
  ...
  F.topRightCorner(3, 1) =
      kindr::getSkewMatrixFromVector(Eigen::Vector3d(0.0, 0.0, 1.0)) *
      R_I_tilde_B.matrix() * v_Delta_t.vector();
