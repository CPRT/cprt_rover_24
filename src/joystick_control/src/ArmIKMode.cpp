...
void ArmIKMode::handleTwist(
  ...
  twist_msg->twist.linear.y = joystickMsg->buttons[kxAxis];
  twist_msg->twist.linear.x = joystickMsg->buttons[kzAxis];
  twist_msg->twist.linear.z = joystickMsg->buttons[kyAxis];

  
  if(joystickMsg->buttons[kBase] == 1 && !swapButton)
  {
    frame_to_publish_ = BASE_FRAME_ID;
    swapButton = true;
  }
  else if(joystickMsg->buttons[kEEF] == 1 && !swapButton)
  {
    frame_to_publish_ = EEF_FRAME_ID;
    swapButton = true;
  }
  else if(!joystickMsg->buttons[kEEF] == 1 && joystickMsg->buttons[kBase] == 1)
  {
    swapButton = false;
  }
...
void ArmIKMode::declareParameters(rclcpp::Node* node) {
    ...
    node->declare_parameter("arm_ik_mode.z_axis", 2);
    node->declare_parameter("arm_ik_mode.rotate_around_y", 3);
    node->declare_parameter("arm_ik_mode.rotate_around_x", 4);
    node->declare_parameter("arm_ik_mode.rotate_around_z", 5);
    node->declare_parameter("arm_ik_mode.open_claw", 6);
  ...
  void ArmIKMode::loadParameters() {
    ...
    node_->get_parameter("arm_ik_mode.z_axis", kzAxis);
