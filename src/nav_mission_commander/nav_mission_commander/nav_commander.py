...
class GpsCommander(Node):
    ...
    def __init__(self):
        ...
        nav_fix_topic = "fromLL"
        geopose_service_name = "gps_goal"

        self.localizer_callback_group = MutuallyExclusiveCallbackGroup()
        self.localizer = self.create_client(
            FromLL, nav_fix_topic, callback_group=self.localizer_callback_group
        )
        self.geopose_service = self.create_service(
            NavToGPSGeopose, geopose_service_name, self.geopose_server
        )
        ...
        while not self.localizer.wait_for_service(timeout_sec=2.0):
            ...
            if count % 10 == 1:
                self.get_logger().info(
                    f"Service on {nav_fix_topic} is not available, waiting again..."
                )
