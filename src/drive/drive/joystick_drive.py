...
class joystickDrive(Node):
    def __init__(self):
        ...
        self.pidMode = self.get_parameter("PID").get_parameter_value().integer_value
    ...
    def joystick_timeout_handler(self):
        ...
        elif Node.get_clock(self).now().seconds_nanoseconds()[
            0
        ] - self.lastTimestamp <= 2 and (self.estopTimeout.data and not self.estop):
            self.estopTimeout.data = False
            self.setEstop.publish(self.estopTimeout)
    ...
    def cmd_joy_callback(self, msg: Joy):
        ...
        if self.pidMode == 1 and self.active:
            self.twist.linear.x = map_range(
                msg.axes[1], -1, 1, -self.MAX_PID_SPEED, self.MAX_PID_SPEED
            )
            self.twist.angular.z = map_range(
                msg.axes[0], -1, 1, -self.MAX_PID_TURN, self.MAX_PID_TURN
            )
            self.setTwistPub.publish(self.twist)
        elif self.active:
            self.twist.linear.x = map_range(
                msg.axes[1], -1, 1, -self.MAX_VOLTAGE_SPEED, self.MAX_VOLTAGE_SPEED
            )
            self.twist.angular.z = map_range(
                msg.axes[0], -1, 1, -self.MAX_VOLTAGE_TURN, self.MAX_VOLTAGE_TURN
            )
            self.setTwistPub.publish(self.twist)
