...
class KeyboardDetector:
    ...
    def __init__(self):
        ...
        path_to_saved_model = os.path.join(self._package_dir, "model/exported-models/saved_model")
        path_to_labels = os.path.join(self._package_dir, "model/annotations/label_map.pbtxt")

        self._class_name_to_int: Dict[str, int] = {}
        self._class_int_to_name: Dict[int, str] = {}
        self._class_int_to_name, self._class_name_to_int = self._load_label_map(path_to_labels)
    ...
    def _load_model(self, path_to_model: str) -> Tuple[Any, tf.function]:
        ...
        detect_fn = detection_model.signatures['serving_default'] # Get the detection function
    ...
    def detect(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        for i in range(num_detections):
            ...
            if class_id in self._class_int_to_name:
                ...
                detections['detection_names'].append("Unknown")
        return detections
    
    def draw_detections_onto_image(self, image: np.ndarray, detections: Dict[str, Any], min_score_thresh: float = 0.3) -> None:
        ...
        for i in range(detections['detection_boxes'].shape[0]):
            ...
            if score > min_score_thresh:
                ...
                cv2.rectangle(image, (int(left), int(top)), (int(right), int(bottom)), (0, 255, 0), 1) # green
                ...
                cv2.putText(image, label, (start_x, start_y), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 1, cv2.LINE_AA) # red
...
if __name__ == "__main__":
    ...
    image = image[int((image.shape[0] - dim[1]) / 2):int((image.shape[0] + dim[1]) / 2),
