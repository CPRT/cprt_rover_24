...
class tfKeyboard(Node):
    ...
    def keyboard_callback(self, request, response):
        ...
        with tf.Session(graph=tf.Graph()) as sess:
            ...
            image_exp_dims = np.expand_dims(frame, axis=0)
            start_time = time.time()
            score, classes, boxes, nums_det, keypoint, keypoint_score = sess.run(
                [
                    det_score,
                    det_class,
                    det_boxes,
            ...
            for i in range(int(nums_det[0])):
                if score[0][i] * 100 > 50:
                    ...
                    p2 = (x2, y2)
                    cv2.rectangle(frame, p1, p2, (0, 255, 0), 3)
            labeled = process_keypoint(
                keypoint[0][0], keypoint_score[0], height, width, frame
            )
            # cv2.imshow("display",frame)
            nparray = np.array(labeled)
            print("Time: ", time.time() - start_time)
            ...
            # print(l)
            topd = math.sqrt(
                (l[1][0] - l[0][0]) * (l[1][0] - l[0][0])
                + (l[1][1] - l[0][1]) * (l[1][1] - l[0][1])
            )
            botd = math.sqrt(
                (l[2][0] - l[3][0]) * (l[2][0] - l[3][0])
                + (l[2][1] - l[3][1]) * (l[2][1] - l[3][1])
            )
            print("Top length (pixels): " + str(topd))
            print(
                "Top distance (estimated) "
                + str(92.4 * math.pow(math.e, -0.00116 * topd))
            )
            print("Bot length: " + str(botd))
            print(
                "Bot dist (estimated) " + str(92.4 * math.pow(math.e, -0.00116 * botd))
            )
            dis = (
                (92.4 * math.pow(math.e, -0.00116 * topd))
                + (92.4 * math.pow(math.e, -0.00116 * botd))
            ) / 2.0
            ...
            red = key_pos_dict[
                data
            ]  # please do not send anything other than invididual lowercase letters, I don't know what happens otherwise but it can't be good.

            purple = red.copy()
            for x in range(0, len(purple)):
                print(
                    f"{x} {red[x]} - {blue[x]} = {red[x] - blue[x]}, {(red[x] - blue[x])/m_dim[x]*keyboard_dim[x]}"
                )
                purple[x] -= blue[x]
                purple[x] = purple[x] / m_dim[x] * keyboard_dim[x]
