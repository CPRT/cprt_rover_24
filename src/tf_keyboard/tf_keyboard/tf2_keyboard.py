...
bs = [
    ...
    (-0.1, 0.1, 18.0),
    (-0.05, 0.05, 18.0),
]  # stands for, well, you know
bsCount = 0

colors = [(255, 0, 0), (229, 52, 235), (235, 85, 52), (14, 115, 51), (14, 115, 204)]
...
class results:
    ...
    def get_likely_keycap_index(self):
        ...
        for x in range(len(self.data["detection_boxes"])):
            point_x = (
                self.data["detection_boxes"][x][1] + self.data["detection_boxes"][x][3]
            ) / 2.0
            point_y = (
                self.data["detection_boxes"][x][0] + self.data["detection_boxes"][x][2]
            ) / 2.0
            dist = math.sqrt(
                (point_x - 0.5) * (point_x - 0.5) + (point_y - 0.5) * (point_y - 0.5)
            ) + 0.5 * (1 - self.data["detection_scores"][x])
    ...
    def check_superior_overlaps(self, k):
        ...
        for x in range(len(self.data["detection_boxes"])):
            ...
            print(
                f'Index {index} with score {self.data["detection_scores"][index]} box {pos} {pos_x} {pos_y} compared to {x} {self.data["detection_scores"][x]} {box} {self.data["detection_classes"][x]}'
            )
            if (
                pos_x > box[1]
                and pos_x < box[3]
                and pos_y > box[0]
                and pos_y < box[2]
                and self.data["detection_classes"][x] != classes["keyboard"]
            ):
                return True

        return False

    def get_likely_keycap_pos(
        self,
    ):  # returns the pixel dimensions of the keycap closest to the center
        closest = self.get_likely_keycap_index()

        print(
            f"Closest = {self.data['detection_boxes'][closest]} id {self.data['detection_classes'][closest]}"
        )
    ...
    def dist_to_key(self, k):
        ...
        print(
            f"{p_x*1280} - {center_x*1280} {p_y*720} - {center_y*720} {ratio_x*1280} {ratio_y*720}"
        )
        print(
            f"Thinks key {key} at {key[0]*720} {key[1]*1280} {key[2]*720} {key[3]*1280} with center {center_x*1280} {center_y*720}"
        )
        print(
            f"Thinks center {p} at {p[0]*720} {p[1]*1280} {p[2]*720} {p[3]*1280} with center {p_x} {p_y}"
        )
        print(
            f"Has {ratio_x*1280} pixels per 1.2 cm and {ratio_y*720} pixels per 1.4 cm"
        )
...
class tf2Keyboard(Node):
    ...
    def keyboard_callback(self, request, response):
        ...
        cap = cv2.VideoCapture(
            "/dev/video4"
        )  # 0 is the default camera (usually the built-in one)
        ...
        k = np.array(
            [
                [817.1125488944084, 0.0, 712.3596465245281],
                [0.0, 758.2820572597227, 400.5753281200634],
                [0.0, 0.0, 1.0],
            ]
        )
        d = np.array(
        ...
        image_np = cv2.warpAffine(
            image_np, world_rotating_matrix, np.array([1280, 720])
        )
