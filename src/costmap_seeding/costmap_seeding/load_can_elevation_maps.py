...
class MapLoader(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("map_square_size_m", 2000.0)  # Default to 500m square
        self.declare_parameter(
        ...
        self.toLL_callback_group = (
            MutuallyExclusiveCallbackGroup()
        )  # Dedicated for toLL client
        self.fromLL_callback_group = (
            MutuallyExclusiveCallbackGroup()
        )  # Dedicated for fromLL client
        ...
        qos_profile_transient = QoSProfile(
            ...
            depth=1,
            durability=DurabilityPolicy.TRANSIENT_LOCAL,  # Added durability policy
        )
        ...
        self.fromLL_client = self.create_client(
            FromLL, "fromLL", callback_group=self.fromLL_callback_group
        )
        # Wait for the fromLL service to be available before proceeding
        self.get_logger().info("Waiting for fromLL service to be available...")
        ...
        self.toLL_client = self.create_client(
            ToLL,
            "toLL",
            callback_group=self.toLL_callback_group,  # Using general service group
        )
        # Wait for the toLL service to be available before proceeding
        self.get_logger().info("Waiting for toLL service to be available...")
        ...
        # especially for service calls. This timer will use the default callback group
        # or a dedicated one if specified.
        self.timer = self.create_timer(
            1.0,
            self._initial_map_loading_timer_callback,
    ...
    def convert_lat_lon_to_pose(
        ...
        try:
            ...
            # Wait for the event with a timeout to prevent indefinite hangs
            if not event.wait(timeout=5.0):  # 5-second timeout
                self.get_logger().warn("Timeout waiting for fromLL service response.")
                if not rclpy.ok():
                    self.get_logger().info(
                        "ROS2 context is shutting down, aborting lat/lon to map conversion."
                    )
                return None
    ...
    def load_maps(self):
        self.get_logger().info("Starting optimized map loading process...")
        ...
        # Loop until a good GPS fix is received.
        while self.latest_gps_fix is None:
            self.get_logger().info("No good GPS fix yet, waiting...")
        ...
        self.get_logger().info("Found the following intersecting maps:")
        i = 1
        for map_data in intersecting_maps_data:
            self.get_logger().info(f"PNG Path {i}: {map_data['png_path']}")
            i += 1

        # 4. Determine the overall bounding box for the combined map
        # The combined map will strictly be map_square_size_m centered on current_map_x, current_map_y
        half_square_size_map = self.map_square_size_m / 2.0
        global_min_x = current_map_x - half_square_size_map
        global_max_x = current_map_x + half_square_size_map
        global_min_y = current_map_y - half_square_size_map
        global_max_y = current_map_y + half_square_size_map

        # Define the target resolution for the final OccupancyGrid
        target_map_resolution = 0.2  # meters/pixel

        # Get the resolution of the source PNG maps (assuming consistent across all)
        # This is assumed to be 1.0 m/pixel based on the user's request for 5x upscaling.
        source_map_resolution = intersecting_maps_data[0]["metadata"]["resolution_m"]

        # Calculate dimensions of the combined OccupancyGrid in pixels at the SOURCE resolution
        combined_width_m = global_max_x - global_min_x
        combined_height_m = global_max_y - global_min_y

        combined_width_pixels_source_res = int(
            math.ceil(combined_width_m / source_map_resolution)
        )
        combined_height_pixels_source_res = int(
            math.ceil(combined_height_m / source_map_resolution)
        )

        self.get_logger().info(
            f"Combined map dimensions (source res): {combined_width_pixels_source_res}x{combined_height_pixels_source_res} pixels at {source_map_resolution}m/pixel."
        )
        self.get_logger().info(
            f"Combined map origin (map frame): X={global_min_x:.2f}, Y={global_min_y:.2f}"
        )

        # Initialize the combined OccupancyGrid data with -1 (unknown) at SOURCE resolution
        combined_occupancy_data_source_res = np.full(
            (combined_height_pixels_source_res, combined_width_pixels_source_res),
            -1,
            dtype=np.int8,
        )

        # 5. Load and combine PNG images into the global OccupancyGrid (Optimized with NumPy)
        #    This part will now work with combined_occupancy_data_source_res
        for map_data in intersecting_maps_data:
            ...
            self.get_logger().info(
                f"Processing map '{map_metadata['filename']}' ({width_img}x{height_img} pixels)."
            )

            # Convert image pixel values (0-255) to OccupancyGrid format (0-100 or -1)
            occupancy_values_from_img = np.full(image.shape, -1, dtype=np.int8)

            known_pixels_mask = image != value_unknown

            # Apply scaling only to known pixels
            occupancy_values_from_img[known_pixels_mask] = np.round(
                (image[known_pixels_mask].astype(float) - value_min)
                / (value_max - value_min)
                * 100.0
            ).astype(np.int8)

            # Ensure values are clamped between 0 and 100 for known pixels
            occupancy_values_from_img[known_pixels_mask] = np.clip(
                occupancy_values_from_img[known_pixels_mask], 0, 100
            )

            # ROS OccupancyGrid has bottom-left origin, OpenCV image has top-left origin.
            # Flip the image vertically to align its coordinate system with ROS's.
            occupancy_values_from_img_ros_oriented = np.flipud(
                occupancy_values_from_img
            )
            ...
            # to the combined map's bottom-left origin (at source resolution).
            offset_x_pixels = int(
                round((current_map_bl_x - global_min_x) / source_map_resolution)
            )
            offset_y_pixels = int(
                round((current_map_bl_y - global_min_y) / source_map_resolution)
            )

            self.get_logger().debug(
                f"Map '{map_metadata['filename']}' pixel offset: ({offset_x_pixels}, {offset_y_pixels})"
            )

            # Determine the intersection region in pixels for both source (current map)
            # and destination (combined map) at SOURCE resolution

            # Destination (combined map) coordinates at source resolution
            dest_x_start = max(0, offset_x_pixels)
            dest_y_start = max(0, offset_y_pixels)
            dest_x_end = min(
                combined_width_pixels_source_res, offset_x_pixels + width_img
            )
            dest_y_end = min(
                combined_height_pixels_source_res, offset_y_pixels + height_img
            )

            # Source (current map) coordinates, adjusted for clipping
            src_x_start = max(0, -offset_x_pixels)
            src_y_start = max(0, -offset_y_pixels)
            src_x_end = min(
                width_img, combined_width_pixels_source_res - offset_x_pixels
            )
            src_y_end = min(
                height_img, combined_height_pixels_source_res - offset_y_pixels
            )

            # Check if there's any valid intersection after clipping
            if dest_x_end <= dest_x_start or dest_y_end <= dest_y_start:
                self.get_logger().debug(
                    f"Map '{map_metadata['filename']}' has no valid intersection with combined map grid after clipping. Skipping."
                )
                continue

            # Get the slices for the destination and source regions
            dest_slice = (
                slice(dest_y_start, dest_y_end),
                slice(dest_x_start, dest_x_end),
            )
            src_slice = (slice(src_y_start, src_y_end), slice(src_x_start, src_x_end))

            # Extract the relevant parts for processing
            current_combined_region = combined_occupancy_data_source_res[dest_slice]
            new_map_region = occupancy_values_from_img_ros_oriented[src_slice]

            # Apply max cost logic using NumPy operations
            # Mask for pixels where the new map has a known value
            new_known_mask = new_map_region != -1
            # Mask for pixels where the combined map currently has an unknown value
            current_unknown_mask = current_combined_region == -1

            # Case 1: New map has a known value, and current combined is unknown -> take new value
            combined_occupancy_data_source_res[dest_slice][
                new_known_mask & current_unknown_mask
            ] = new_map_region[new_known_mask & current_unknown_mask]

            # Case 2: Both new map and current combined have known values -> take maximum
            both_known_mask = (
                new_known_mask & ~current_unknown_mask
            )  # ~current_unknown_mask means current is known
            combined_occupancy_data_source_res[dest_slice][both_known_mask] = (
                np.maximum(
                    current_combined_region[both_known_mask],
                    new_map_region[both_known_mask],
                )
            )
            # Other cases (new unknown, current known; both unknown) are implicitly handled:
            # - If new is unknown, and current is known, current value is kept.
            # - If both are unknown, value remains -1.

        # --- NEW CODE FOR UPSAMPLING ---
        # Calculate the scaling factor based on source and target resolutions
        scaling_factor = source_map_resolution / target_map_resolution
        if not scaling_factor.is_integer():
            self.get_logger().warn(
                f"Scaling factor is not an integer ({scaling_factor}). This might lead to unexpected resizing behavior for occupancy grids."
            )
        scaling_factor = int(
            scaling_factor
        )  # Ensure it's an integer for pixel operations

        if scaling_factor > 1:
            self.get_logger().info(
                f"Upsampling combined map from {source_map_resolution}m/pixel to {target_map_resolution}m/pixel (factor: {scaling_factor}x)."
            )
            # Resize the combined map to the target resolution
            # Use INTER_NEAREST to preserve discrete occupancy values (0-100, -1)
            combined_occupancy_data_target_res = cv2.resize(
                combined_occupancy_data_source_res,
                (
                    combined_width_pixels_source_res * scaling_factor,
                    combined_height_pixels_source_res * scaling_factor,
                ),
                interpolation=cv2.INTER_NEAREST,
            )
            # Ensure the data type remains int8 after resize
            combined_occupancy_data = combined_occupancy_data_target_res.astype(np.int8)
        else:
            self.get_logger().info("No upsampling needed (scaling factor <= 1).")
            combined_occupancy_data = combined_occupancy_data_source_res

        # Update final dimensions for the OccupancyGrid message
        combined_height_pixels, combined_width_pixels = combined_occupancy_data.shape

        self.get_logger().info(
            f"Final combined map dimensions (target res): {combined_width_pixels}x{combined_height_pixels} pixels at {target_map_resolution}m/pixel."
        )
        ...
        occupancy_grid_msg.info.resolution = float(
