...
def list_dtm_urls_from_kml(base_directory_url, kml_filename="INDEX_utm12_AB_Drumheller.kmz", local_kml_path="kml"):
    """
...
def download_dtm_data_from_kml_index(base_data_directory_url, local_kml_folder, output_dtm_folder):
    """
    ...
    """
    all_dtm_tif_urls = list_dtm_urls_from_kml(base_data_directory_url, local_kml_path=local_kml_folder)
...
def calculate_traversability_cost(
    ...
    if non_nan_rows.size == 0 or non_nan_cols.size == 0:
        print("Warning: Input elevation data contains only NaN values. No costmap will be generated with valid data.")
        ...
        center_x_orig, center_y_orig = original_transform * (elevation_data_2d.shape[1] / 2, elevation_data_2d.shape[0] / 2)
        ...
        new_top_left_y = center_y_orig + (original_resolution_m / 2) # Y increases upwards, so add to top-left

        adjusted_transform = Affine(original_resolution_m, 0, new_top_left_x,
                                    0, -original_resolution_m, new_top_left_y)
    ...
    elevation_data_cropped = elevation_data_2d[min_row_crop:max_row_crop+1, min_col_crop:max_col_crop+1]
    
    # Calculate the top-left coordinate of the cropped area in the original CRS (UTM)
    new_top_left_utm_x, new_top_left_utm_y = original_transform * (min_col_crop, min_row_crop)
    
    # Create the adjusted transform for the cropped array
    adjusted_transform = Affine(original_resolution_m, 0, new_top_left_utm_x,
                                0, -original_resolution_m, new_top_left_utm_y)

    rows, cols = elevation_data_cropped.shape
    print(f"Elevation data cropped to {rows}x{cols} from original. Proceeding with calculations on cropped data.")
    ...
    if max_chunk_dimension < 2 * PAD_PIXELS + 1:
        ...
        print(f"Warning: max_chunk_dimension ({max_chunk_dimension}) is too small for padding ({PAD_PIXELS}). Using effective chunk dimension of {MAX_CHUNK_DIM_EFFECTIVE}.")
    else:
    ...
    if rows * cols > MAX_CHUNK_DIM_EFFECTIVE * MAX_CHUNK_DIM_EFFECTIVE:
        print(f"Cropped array size ({rows}x{cols}) exceeds max chunk size ({MAX_CHUNK_DIM_EFFECTIVE}x{MAX_CHUNK_DIM_EFFECTIVE}). Processing in chunks.")
        
        # Initialize output cost array at original resolution
        combined_cost_original_res = np.empty_like(elevation_data_cropped, dtype=np.float32)
        ...
        if effective_chunk_dim <= 0:
            print("Error: Effective chunk dimension is too small after accounting for padding. Increase max_chunk_dimension or decrease roughness_blur_sigma.")
            return None, None, None
        ...
        for r_start in range(0, rows, effective_chunk_dim):
            for c_start in range(0, cols, effective_chunk_dim):
                ...
                current_elevation_chunk = elevation_data_cropped[padded_r_start:padded_r_end, padded_c_start:padded_c_end]
                ...
                dz_dy_chunk, dz_dx_chunk = np.gradient(current_elevation_chunk, original_resolution_m)
                slope_radians_chunk = np.arctan(np.sqrt(dz_dx_chunk**2 + dz_dy_chunk**2))
                slope_degrees_chunk = np.degrees(slope_radians_chunk)
                slope_cost_chunk = np.clip(slope_degrees_chunk / max_slope_deg, 0.0, 1.0)

                # Calculate roughness for the chunk
                smoothed_elevation_chunk = scipy.ndimage.gaussian_filter(current_elevation_chunk, sigma=roughness_blur_sigma, mode='constant', cval=np.nan)
                roughness_chunk = np.abs(current_elevation_chunk - smoothed_elevation_chunk)
                roughness_cost_chunk = np.clip(roughness_chunk / max_roughness_m, 0.0, 1.0)
                ...
                if total_weight == 0:
                    ...
                    chunk_combined_cost = (normalized_slope_weight * slope_cost_chunk) + \
                ...
                combined_cost_original_res[r_start:r_end, c_start:c_end] = \
                    chunk_combined_cost[crop_r_start:crop_r_end, crop_c_start:crop_c_end]
        print(f"Cost map generated in chunks at original {original_resolution_m}m resolution.")
    else: # No chunking needed, process the whole array directly
        print(f"Cropped array size ({rows}x{cols}) is within max chunk size. Processing as a whole.")
        # 1. Calculate Slope
        ...
        print(f"Slope cost calculated. Min: {np.nanmin(slope_cost) if not np.all(np.isnan(slope_cost)) else 'NaN' :.2f}, Max: {np.nanmax(slope_cost) if not np.all(np.isnan(slope_cost)) else 'NaN' :.2f}")

        # 2. Calculate Roughness
        smoothed_elevation = scipy.ndimage.gaussian_filter(elevation_data_cropped, sigma=roughness_blur_sigma, mode='constant', cval=np.nan)
        roughness = np.abs(elevation_data_cropped - smoothed_elevation)
        roughness_cost = np.clip(roughness / max_roughness_m, 0.0, 1.0)
        print(f"Roughness cost calculated. Min: {np.nanmin(roughness_cost) if not np.all(np.isnan(roughness_cost)) else 'NaN' :.2f}, Max: {np.nanmax(roughness_cost) if not np.all(np.isnan(roughness_cost)) else 'NaN' :.2f}")

        # 3. Combine the cost of slope with the cost of roughness
        total_weight = slope_weight + roughness_weight
        if total_weight == 0:
            print("Warning: Both slope_weight and roughness_weight are zero. Returning zero cost map with NaNs retained.")
            combined_cost_original_res = np.zeros_like(slope_cost) # Still need to apply NaN mask later
        else:
            normalized_slope_weight = slope_weight / total_weight
            normalized_roughness_weight = roughness_weight / total_weight
    ...
    print(f"Final combined cost (at original resolution). Min: {np.nanmin(combined_cost_original_res) if not np.all(np.isnan(combined_cost_original_res)) else 'NaN' :.2f}, Max: {np.nanmax(combined_cost_original_res) if not np.all(np.isnan(combined_cost_original_res)) else 'NaN' :.2f}")
    ...
    if original_resolution_m != target_resolution_m:
        ...
        output_cost_array = scipy.ndimage.zoom(combined_cost_original_res, zoom_factor, order=3, mode='constant', cval=np.nan)
        ...
        adjusted_transform = Affine(target_resolution_m, adjusted_transform.b, adjusted_transform.c,
                                    adjusted_transform.d, -target_resolution_m, adjusted_transform.f)
...
def write_elevation_costmap(cost_array, costmap_origin_transform, costmap_crs, output_filepath_stem, resolution_m):
    """
    ...
    cost_array_processed[np.isnan(cost_array_processed)] = UNKNOWN_VALUE_PNG + 1 # Temporarily set to > 255 for scaling
    ...
    print(f"Costmap image saved to: {image_filename} (dimensions: {scaled_cost_array.shape[0]}x{scaled_cost_array.shape[1]})")

    # Prepare and save YAML metadata
    rows, cols = scaled_cost_array.shape # Use shape of the actual saved image

    minx, maxy = costmap_origin_transform * (0, 0) # Top-left (col, row) -> (x, y) of the *cropped* image
    maxx, miny = costmap_origin_transform * (cols, rows) # Bottom-right (col, row) -> (x, y) of the *cropped* image
    ...
    metadata = {
        ...
        'value_scale': {'min': 0, 'max': 254, 'unknown': UNKNOWN_VALUE_PNG}, # Added unknown value
        'corners_lat_lon': {
...
class CanElevationDownloader(Node): # Renamed node
    def __init__(self):
        ...
        self.dataset_parent_dir = self.get_parameter('dataset_parent_dir').get_parameter_value().string_value

        # Check if the dataset_parent_dir exists and has usable permissions
        if not os.path.exists(self.dataset_parent_dir):
            self.get_logger().error(f"Base directory '{self.dataset_parent_dir}' does not exist. Please create it and ensure permissions.")
            # Depending on desired behavior, you might want to raise an exception or rclpy.shutdown() here
        elif not os.access(self.dataset_parent_dir, os.W_OK | os.X_OK): # Check for write and execute permissions
            self.get_logger().error(f"Base directory '{self.dataset_parent_dir}' does not have sufficient write/execute permissions. Please adjust permissions.")
            # Depending on desired behavior, you might want to raise an exception or rclpy.shutdown() here
        else:
            self.get_logger().info(f"Base directory '{self.dataset_parent_dir}' exists and has usable permissions.")

        # Set the full data base directory based on the parameter
        self.data_base_dir = os.path.join(self.dataset_parent_dir, 'elevation_datasets', 'CanElevation')
        ...
        self.declare_parameter('dataset_names', ['Drumheller']) # Changed to 'Drumheller'
        self.declare_parameter('Drumheller_url', "https://ftp.maps.canada.ca/pub/elevation/dem_mne/highresolution_hauteresolution/dtm_mnt/1m/AB/Drumheller/") # Changed parameter name
        ...
        self.declare_parameter('max_chunk_dimension', 6000) # New parameter for max chunk size
        ...
        self.target_map_resolution_meters = self.get_parameter('target_map_resolution_meters').get_parameter_value().double_value
        self.max_chunk_dimension = self.get_parameter('max_chunk_dimension').get_parameter_value().integer_value
        self.slope_weight = self.get_parameter('slope_weight').get_parameter_value().double_value
        self.roughness_weight = self.get_parameter('roughness_weight').get_parameter_value().double_value
        self.max_slope_deg = self.get_parameter('max_slope_deg').get_parameter_value().double_value
        self.max_roughness_m = self.get_parameter('max_roughness_m').get_parameter_value().double_value
        self.roughness_blur_sigma = self.get_parameter('roughness_blur_sigma').get_parameter_value().double_value
        ...
        self.generate_costmaps_srv = self.create_service(Trigger, 'generate_elevation_costmaps', self.generate_elevation_costmaps_callback, qos_profile=qos_profile)
    ...
    def download_all_dtms(self):
        ...
        for dataset_name in self.dataset_names:
            ...
            try:
                ...
                kml_output_folder = os.path.join(dataset_base_folder, "kml") # KML in dedicated 'kml' folder
                dtm_output_folder = os.path.join(dataset_base_folder, "dtm") # DTMs in dedicated 'dtm' folder
                ...
                self.get_logger().info(f"Downloading DTM files for {dataset_name} to {dtm_output_folder}...")
                ...
                if downloaded_filepaths:
                    ...
                    dtm_info = {
                        ...
                        'dtm_files': [os.path.relpath(fp, dtm_output_folder) for fp in downloaded_filepaths] # Store relative paths
                    }
    ...
    def generate_elevation_costmaps_callback(self, request, response):
        ...
        try:
            ...
            for dataset_name in self.dataset_names:
                self.get_logger().info(f"Generating costmaps for dataset: {dataset_name}")
                
                dataset_base_folder = os.path.join(self.data_base_dir, dataset_name)
                dtm_input_folder = os.path.join(dataset_base_folder, "dtm") # DTMs are now in 'dtm' subfolder
                dtm_index_path = os.path.join(dataset_base_folder, "dtm_index.yaml") # Index is at dataset root
                costmap_output_dir = os.path.join(dataset_base_folder, "costmaps")
                os.makedirs(costmap_output_dir, exist_ok=True) # Ensure costmaps directory exists

                if not os.path.exists(dtm_index_path):
                    self.get_logger().warning(f"DTM index file not found for {dataset_name} at {dtm_index_path}. Skipping costmap generation for this dataset.")
                    continue
                ...
                try:
                    with open(dtm_index_path, 'r') as f:
                        ...
                        if 'dtm_files' in dtm_info:
                            ...
                            downloaded_filepaths = [os.path.join(dtm_input_folder, f) for f in dtm_info['dtm_files']]
                        else:
                            self.get_logger().error(f"dtm_files key not found in {dtm_index_path}. Skipping.")
                            continue
                except Exception as e:
                    self.get_logger().error(f"Error loading DTM index file for {dataset_name}: {e}. Skipping.")
                    continue

                if not downloaded_filepaths:
                    self.get_logger().warning(f"No DTM files listed in {dtm_index_path}. Skipping costmap generation for this dataset.")
                    continue

                # 3. Iterate over all DTM files in the dataset
                for dtm_filepath in downloaded_filepaths:
                    if not os.path.exists(dtm_filepath):
                        self.get_logger().warning(f"DTM file {dtm_filepath} not found. Skipping costmap generation for this tile.")
                        continue

                    self.get_logger().info(f"Processing DTM tile: {os.path.basename(dtm_filepath)}")
                    try:
                        with rasterio.open(dtm_filepath) as src:
                            elevation_array_tile = src.read(1)
                            original_resolution_m = src.res[0] # Assuming square pixels, res is (width, height)
                            ...
                            if no_data_value is not None:
                                elevation_array_tile = np.where(elevation_array_tile == no_data_value, np.nan, elevation_array_tile)
                                
                                # Print number of NaNs for debugging
                                num_nans = np.sum(np.isnan(elevation_array_tile))
                                self.get_logger().info(f"Tile {os.path.basename(dtm_filepath)} contains {num_nans} NaN values (no_data).")

                            # Call calculate_traversability_cost with ROS2 parameters
                            traversability_cost_map, adjusted_transform, adjusted_crs = calculate_traversability_cost(
                            ...
                            if traversability_cost_map is not None:
                                tile_filename_stem = os.path.splitext(os.path.basename(dtm_filepath))[0]
                                output_filepath_stem = os.path.join(costmap_output_dir, tile_filename_stem)
                                ...
                                self.get_logger().warning(f"Failed to calculate traversability cost map for {dtm_filepath}. Possibly all data was NaN.")
