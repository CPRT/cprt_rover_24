...
# --- Utility Functions (now methods of CanElevationDownloader) ---
...
class CanElevationDownloader(Node):  # Renamed node
    ...
    def _download_file_from_url(self, url, destination_folder):
        """
        ...
        try:
            ...
            self.get_logger().info(f"Successfully downloaded '{filename}'")
            return filename
            ...
            self.get_logger().error(
                f"An unexpected error occurred during download of {filename}: {e}"
            )
            return None

    def _list_dtm_urls_from_kml(
        self,
        base_directory_url,
        ...
        kmz_local_path = self._download_file_from_url(kml_url, local_kml_path)
        if not kmz_local_path:
            self.get_logger().error(f"Failed to download KMZ file from {kml_url}")
            return []
        ...
        try:
            with zipfile.ZipFile(kmz_local_path, "r") as zip_ref:
                ...
                kml_files = [
                    f for f in zip_ref.namelist() if f.lower().endswith(".kml")
                ]
                if not kml_files:
                    self.get_logger().warning(
                        f"No KML file found inside {kmz_local_path}"
                    )
                    return []
                ...
                self.get_logger().info(f"Extracted KML to: {kml_extracted_path}")
        except zipfile.BadZipFile:
            self.get_logger().error(f"Error: {kmz_local_path} is not a valid KMZ file.")
            return []
        except Exception as e:
            self.get_logger().error(f"Error extracting KML from KMZ: {e}")
            return []
    ...
    def _download_dtm_data_from_kml_index(
        self, base_data_directory_url, local_kml_folder, output_dtm_folder
    ):
        """
        Downloads all available DTM GeoTIFF files based on a KML index.
        Automatically discovers the .kmz file in the base_data_directory_url.
        ...
        """
        # 1. Discover the KMZ file from the base_data_directory_url
        kmz_filename = None
        try:
            response = requests.get(base_data_directory_url)
            response.raise_for_status()
            soup = BeautifulSoup(response.text, "html.parser")
            for link in soup.find_all("a", href=True):
                href = link["href"]
                if href.lower().endswith(".kmz"):
                    kmz_filename = href
                    self.get_logger().info(f"Discovered KMZ file: {kmz_filename}")
                    break
            if not kmz_filename:
                self.get_logger().error(
                    f"No .kmz file found at {base_data_directory_url}. Cannot proceed with DTM download."
                )
                return []
        except requests.exceptions.RequestException as e:
            self.get_logger().error(
                f"Error accessing {base_data_directory_url} to find KMZ: {e}"
            )
            return []
        except Exception as e:
            self.get_logger().error(
                f"An unexpected error occurred while discovering KMZ: {e}"
            )
            return []

        all_dtm_tif_urls = self._list_dtm_urls_from_kml(
            base_data_directory_url,
            kml_filename=kmz_filename,  # Pass the discovered KMZ filename
            local_kml_path=local_kml_folder,
        )

        if not all_dtm_tif_urls:
            self.get_logger().warning(
                "No DTM .tif URLs found from KMZ. Cannot download any data."
            )
            return []
        ...
        for tif_url in all_dtm_tif_urls:
            downloaded_path = self._download_file_from_url(tif_url, output_dtm_folder)
            if downloaded_path:
                downloaded_filepaths.append(downloaded_path)

        return downloaded_filepaths

    def _calculate_traversability_cost(
        self,
        elevation_data_2d,
        ...
        if non_nan_rows.size == 0 or non_nan_cols.size == 0:
            self.get_logger().warning(
                "Input elevation data contains only NaN values. No costmap will be generated with valid data."
            )
        ...
        if max_chunk_dimension < 2 * PAD_PIXELS + 1:
            ...
            self.get_logger().warning(
                f"max_chunk_dimension ({max_chunk_dimension}) is too small for padding ({PAD_PIXELS}). Using effective chunk dimension of {MAX_CHUNK_DIM_EFFECTIVE}."
            )
        else:
    ...
    def _write_elevation_costmap(
        ...
        if cost_array is None or cost_array.size == 0:
            self.get_logger().warning("No cost array provided for writing.")
            return
        ...
        self.get_logger().info(f"Costmap metadata saved to: {yaml_filename}")
    ...
    def download_all_dtms(self):
        ...
        for dataset_name in self.dataset_names:
            ...
            try:
                ...
                downloaded_filepaths = self._download_dtm_data_from_kml_index(
                    base_data_directory_url,
                    ...
                    try:
                        with rasterio.open(dtm_filepath) as src:
                            ...
                            (
                                ...
                                adjusted_transform,
                                adjusted_crs,
                            ) = self._calculate_traversability_cost(
                                elevation_array_tile,
                                original_resolution_m,
