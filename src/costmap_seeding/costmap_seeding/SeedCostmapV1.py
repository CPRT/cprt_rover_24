...
def calculate_traversability_cost(
    ...
    if np.any(nan_mask):
        ...
        if valid_values.size > 0:
            ...
            return np.full(
                elevation_data_clean.shape, 1.0, dtype=np.float32
            )  # Return high cost
    ...
    print(
        f"Slope cost calculated. Min: {np.min(slope_cost):.2f}, Max: {np.max(slope_cost):.2f}"
    )

    # 2. Calculate Roughness
    # Blur the elevations
    smoothed_elevation = scipy.ndimage.gaussian_filter(
        elevation_data_clean, sigma=roughness_blur_sigma
    )
    ...
    print(
        f"Roughness cost calculated. Min: {np.min(roughness_cost):.2f}, Max: {np.max(roughness_cost):.2f}"
    )
    ...
    if total_weight == 0:
        print(
            "Warning: Both slope_weight and roughness_weight are zero. Returning zero cost map."
        )
        ...
        combined_cost = (normalized_slope_weight * slope_cost) + (
    ...
    print(
        f"Combined cost calculated. Min: {np.min(combined_cost):.2f}, Max: {np.max(combined_cost):.2f}"
    )
    ...
    if original_resolution_m != target_resolution_m:
        ...
        output_cost_array = scipy.ndimage.zoom(
            combined_cost, zoom_factor, order=3
    ...
    def __init__(self):
        ...
        if not os.path.exists(CPRT_ROOT_DIR):
            raise FileNotFoundError(
                f"Base directory '{CPRT_ROOT_DIR}' does not exist. Please create it and ensure permissions."
            )

        elif not os.access(
            CPRT_ROOT_DIR, os.W_OK | os.X_OK
        ):  # Check for write and execute permissions
            raise PermissionError(
                f"Base directory '{CPRT_ROOT_DIR}' does not have sufficient write/execute permissions. Please adjust permissions."
            )
        else:
            self.get_logger().info(
                f"Base directory '{CPRT_ROOT_DIR}' exists and has usable permissions."
            )

        # Set the data base directory to the specified path structure
        self.data_base_dir = os.path.join(
            CPRT_ROOT_DIR, "elevation_datasets", "CanElevation"
        )
        os.makedirs(
        ...
        self.srv = self.create_service(
            Trigger, "process_drumheller_dtm", self.process_drumheller_dtm_callback
    ...
    def process_drumheller_dtm_callback(self, request, response):
        ...
        try:
            ...
            if elevation_array is not None:
                ...
                target_cost_map_res = (
                    0.5  # New resolution for cost map (e.g., 0.5 meters)
                )
