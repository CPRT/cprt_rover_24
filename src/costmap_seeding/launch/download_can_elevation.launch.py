...
def generate_launch_description():
    ...
    param_file_path = os.path.join(
        costmap_seeding_share_dir,
        'config',
        'can_elevation_downloader_params.yaml'
    )
    ...
    can_elevation_downloader_node = Node(
        ...
        parameters=[LaunchConfiguration('params_file')], # Load parameters from the specified YAML file
    )

    # Create the launch description and add the actions
    return LaunchDescription([
        declare_params_file_cmd,
        can_elevation_downloader_node
    ])
