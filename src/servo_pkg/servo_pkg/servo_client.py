...
class Servo_Client(Node):
    def __init__(self):
        ...
        self.servos = self.get_parameter("servos_used").get_parameter_value().integer_value
        self.get_logger().info(f"# of servos: {self.servos}")
        
        self.declare_parameter(f"servo{self.servo}", f"servo{self.servo}")
        self.motor_names = self.get_parameter(f"servo{self.servo}.name").get_parameter_value().string_value 
        self.pub = self.create_publisher(Float32, f"servo{self.servo}.name", 1) # publishes pwm signal integer
        timer_period = 0.5
