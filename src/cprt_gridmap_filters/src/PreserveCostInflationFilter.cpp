...
template<typename T>
PreserveCostInflationFilter<T>::PreserveCostInflationFilter() : method_(Method::RadialInflationSerial), inflationRadius_(0.0)
{
  ///
}
...
bool PreserveCostInflationFilter<T>::configure()
{
  ...
  if (!param_reader.get(std::string("inflation_radius"), this->inflationRadius_)) {
    RCLCPP_ERROR(
      this->logging_interface_->get_logger(), 
      "PreserveCostInflationFilter did not find parameter 'inflation_radius'.");
    return false;
  }
...
bool PreserveCostInflationFilter<T>::update(const T & mapIn, T & mapOut)
{
  ...
  if (!mapOut.exists(this->inputLayer_)) {
    RCLCPP_ERROR(
      this->logging_interface_->get_logger(), "Layer %s does not exist. Unable to apply preserve cost inflation filter.",
  ...
  //   RCLCPP_INFO(this->logging_interface_->get_logger(), "Cell at position (20, 20) is valid and has value %f.",
  //               data(20, 20));
  // }
  // else
  // {
  //   RCLCPP_INFO(this->logging_interface_->get_logger(), "Cell at position (20, 20) is invalid and has value %f.",
  //               data(20, 20));
  // }
...
template<typename T>
void PreserveCostInflationFilter<T>::computeWithSimpleSerialMethod(const grid_map::GridMap &mapIn, grid_map::GridMap &mapOut)
{
  rclcpp::Clock clock;
  ...
  for (grid_map::GridMapIterator iterator(mapIn); !iterator.isPastEnd(); ++iterator) 
  {
    ...
    mapIn.getPosition(index, position); // Get position of cell from grid_map::Index of cell

    
    // RCLCPP_INFO(
    //   this->logging_interface_->get_logger(), "Value: %f", layerIn.coeff(index(0), index(1)));

    this->radialInflateSerial(mapOut, position, layerIn.coeff(index(0), index(1)));
  }
  ...
  RCLCPP_INFO(
    this->logging_interface_->get_logger(), "Preserve Inflation Simple Serial COMPUTATION TIME = %f",
    (end - start));
}

template<typename T>
void PreserveCostInflationFilter<T>::radialInflateSerial(grid_map::GridMap &mapOut, const grid_map::Position &position, const float value)
{
  Eigen::MatrixXf &layerOut = mapOut[this->outputLayer_];

  for (grid_map::CircleIterator iterator(mapOut, position, this->inflationRadius_); !iterator.isPastEnd(); ++iterator) 
  {
    if (!mapOut.isValid(*iterator, this->outputLayer_)) 
    {
      continue;
    ...
    layerOut.coeffRef((*iterator)(0), (*iterator)(1)) = std::min(layerOut.coeff((*iterator)(0), (*iterator)(1)), value);
  }
}
...
PLUGINLIB_EXPORT_CLASS(
  grid_map::PreserveCostInflationFilter<grid_map::GridMap>,
