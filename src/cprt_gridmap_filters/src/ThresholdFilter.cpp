...
bool ThresholdFilter<T>::configure() {
  ...
  if (param_reader.get(std::string("lower_threshold"), lowerThreshold_)) {
    ...
    RCLCPP_DEBUG(this->logging_interface_->get_logger(), "lower threshold = %f",
                 lowerThreshold_);
  }

  if (param_reader.get(std::string("upper_threshold"), upperThreshold_)) {
    useUpperThreshold_ = true;
    RCLCPP_DEBUG(this->logging_interface_->get_logger(), "upper threshold = %f",
                 upperThreshold_);
  }

  if (!useLowerThreshold_ && !useUpperThreshold_) {
    RCLCPP_ERROR(this->logging_interface_->get_logger(),
                 "ThresholdFilter did not find parameter 'lower_threshold' or "
                 "'upper_threshold',");
    return false;
  }
  ...
  if (useLowerThreshold_ && useUpperThreshold_) {
    RCLCPP_ERROR(this->logging_interface_->get_logger(),
                 "Set either 'lower_threshold' or 'upper_threshold'! Only one "
...
bool ThresholdFilter<T>::update(const T& mapIn, T& mapOut) {
  ...
  for (grid_map::GridMapIterator iterator(mapOut); !iterator.isPastEnd();
       ++iterator) {
    if (!mapOut.isValid(*iterator, layer_)) {
      continue;
    }
...
PLUGINLIB_EXPORT_CLASS(grid_map::ThresholdFilter<grid_map::GridMap>,
