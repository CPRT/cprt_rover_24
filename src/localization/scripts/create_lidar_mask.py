...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if flags > 0:  # Scroll up (zoom in)
            ...
            new_zoom_level = max(
                0.1, zoom_level / 1.2
            )  # Min zoom level 1x (original size)
        ...
        if new_zoom_level != zoom_level:
            ...
            current_win_width = (
                ...
                else original_image.shape[1]
            )
            current_win_height = (
                display_image.shape[0]
                if display_image is not None
                else original_image.shape[0]
            )
...
def update_display():
    ...
    processed_image = np.zeros(
        (original_image.shape[0], original_image.shape[1], 3), dtype=np.uint8
    )
    ...
    if np.any(mask):  # Use np.any to check if mask contains any True values
        ...
        if max_val == min_val:
            ...
            colormap_input_values = np.full_like(
                values_in_range, 127, dtype=np.uint8
            )  # A single mid-gray value for colormap
        else:
            ...
            if (
                ...
                print(
                    "Warning: Min/Max values for heatmap are NaN/Inf. Outputting black for heatmap."
                )
            else:
    ...
    cropped_image = processed_image[
        view_y : view_y + current_height_in_orig,
        view_x : view_x + current_width_in_orig,
    ]
...
def load_image(path):
    ...
    if img.dtype == np.uint8:
        ...
        print(
            f"Warning: Unexpected image data type: {img.dtype}. Treating as 16-bit. Max threshold set to 65535."
        )
        lower_threshold = 0
        upper_threshold = 65535
        cv2.setTrackbarMax("Upper Threshold", window_name, 65535)
...
if __name__ == "__main__":
    ...
    print("Instructions:")
    print(
        "  - Use 'Lower Threshold' and 'Upper Threshold' sliders to define the range of pixel values."
    )
    print("  - Pixels outside this range will be black.")
    print(
        "  - Pixels within this range will be displayed with a JET heatmap based on their value."
    )
    ...
    while True:
        ...
        if key == ord("q") or key == 27:  # 'q' or ESC key to exit
            ...
            if original_image is not None:
                ...
                mask_within_threshold = (original_image >= lower_threshold) & (
                    original_image <= upper_threshold
                )
                ...
                if display_image is not None:  # Ensure display_image has been generated
                    heatmap_save_path = os.path.join(
                        save_dir, f"{name_without_ext}_heatmap_view.png"
                    )
