...
class OusterRangeImageSaverSingle(Node):
    def __init__(self):
        ...
        if not os.path.exists(self.save_directory):
            try:
                ...
                self.get_logger().error(
                    f"Error creating directory {self.save_directory}: {e}"
                )
                # Fallback to current working directory if creation fails
                self.save_directory = os.getcwd()
                self.get_logger().warn(
                    f"Saving to current working directory: {self.save_directory}"
                )
        ...
        self.get_logger().info(
            f"Ouster Range Image Saver (Single) node started. Will save one image to: {self.save_directory}"
        )
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            cv_image_16bit = self.bridge.imgmsg_to_cv2(
            ...
            if msg.encoding == "mono16":
                self.get_logger().info(
                    "Image encoding is mono16. Scaling to 8-bit for visualization."
                )
                ...
                cv_image_8bit = cv2.normalize(
                    cv_image_16bit, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U
                )
                ...
                self.get_logger().warn(
                    f"Unsupported image encoding: {msg.encoding}. Attempting to convert to mono8 directly."
                )
