        ...
        while True:
            ...
            for name in list(nodes.keys()):
                ...
                if proc.poll() is not None:
                    logger(f"Node '{name}' terminated unexpectedly with code {proc.poll()}.")
                    if node_info.respawn:
                        node_info.retries += 1
                        delay = min(5, 2 * node_info.retries)
                        logger(f"Respawning '{name}' in {delay} seconds (attempt {node_info.retries}).")
                        _ = nodes.pop(name)
                        threading.Timer(delay, lambda ni=node_info: respawn_callback(ni)).start()
                    else:
                    ...
                    if node_info.mode == "launch":
                        try:
                            ...
                            if recorded_pids and (recorded_pids - current_pids):
                                logger(f"One or more child processes of '{name}' died unexpectedly. Restarting group.")
                                _ = nodes.pop(name)
                                threading.Timer(2.0, lambda ni=node_info: respawn_callback(ni)).start()
